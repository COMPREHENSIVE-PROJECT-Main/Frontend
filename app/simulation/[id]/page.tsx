...
export default function SimulationPage() {
  ...
  // SSE 스트리밍 연결 (React StrictMode 이중 실행 방지 포함)
  useEffect(() => {
    ...
    const startSimulationStream = async () => {
      try {
        ...
        const requestedCaseType = new URLSearchParams(window.location.search).get("case_type") || sessionStorage.getItem("caseType") || "형사";
              ...
              switch (event) {
                ...
                case "token":
                  setLogs(prev => {
                    ...
                    if (pendingRoundRef.current) {
                      const pendingLog = { ...pendingRoundRef.current, msg: text, pending: false };
                ...
                case "round_end":
                  setLogs(prev => {
                    ...
                    const last = prev[prev.length - 1];
                    if (last && last.type === "argument" && last.isStreaming && last.round === data.round && last.speaker === data.speaker) {
        ...
        <div className="max-w-4xl mx-auto flex justify-center items-center">
          {isFinished ? (
            <motion.button
              ...
              animate={{ scale: 1, y: 0 }}
              onClick={() => {
                const caseType = new URLSearchParams(window.location.search).get("case_type") || sessionStorage.getItem("caseType") || "형사";
